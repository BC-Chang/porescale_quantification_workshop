...
def find_porosity_visualization_interval_c(cnp.ndarray[cnp.uint8_t, ndim=3] image, int cube_size=100, int batch=100):
    ...
    cdef int i, max_dim, counts_inside_sum, counts_outside_sum, best_index
    cdef Py_ssize_t mini, maxi, best_interval_index, inc
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=3, cast=True] labels_out_inside
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=1, cast=True] index_inside
    cdef cnp.ndarray[cnp.int64_t, ndim=1, cast=True] counts_outside, counts_inside
    ...
    best_interval_index = int(stats_array[3, best_index])
